...
export default function CurricularApprovalPanel({
  ...
  const cargar = useCallback(async () => {
    ...
    try {
      ...
      setError(
        errorCarga.message ||
          "No se pudieron cargar las propuestas curriculares.",
      );
      setFilas([]);
    } finally {
      setCargando(false);
  ...
  const filasVisibles = useMemo(() => {
    ...
    return (filas || []).filter(
      (fila) =>
        coincideFiltro(fila, filtro) &&
        (!query || textoBuscable(fila).includes(query)),
    );
  ...
  const guardar = async () => {
    ...
    try {
      const datos = revision
        ? await decidirPendientesNormalizador(
            idEjecucion,
            solicitud,
            "ejecutor",
            revision,
      ...
      setError(
        errorGuardado.message ||
          "No se pudieron guardar las decisiones curriculares.",
      );
    } finally {
      setGuardando(false);
    }
  ...
  const aplicarDecisionTecnica = async (idPendiente, decision, reason = "") => {
    ...
    if (!revision) {
      setError(
        "La revisión técnica no está disponible; recarga la cola antes de decidir.",
      );
      return false;
    }
    setGuardando(true);
    ...
    try {
      ...
      setError(
        errorDecision.message ||
          "No se pudo aplicar la decisión técnica inmediatamente.",
      );
      return false;
    } finally {
      setGuardando(false);
  ...
  if (cargando && !filas) {
    return (
      <section
        className="mt-5 rounded-2xl border border-line bg-paper p-5 font-body shadow-sm"
        aria-label="Revisión curricular"
      >
        <div className="flex items-center gap-2 text-sm font-bold text-muted">
          <LoaderCircle
            className="animate-girar text-ulima"
            size={17}
            aria-hidden="true"
          />
  ...
  if (
    ...
    !error &&
    !requiereDecision &&
    !resumen?.remaining_pending
  )
    return null;

  return (
    <section
      className="mt-5 rounded-2xl border border-line border-t-4 border-t-ulima bg-paper p-5 font-body shadow-sm sm:p-6"
      aria-label="Aprobación de propuestas curriculares"
    >
      ...
      <div
        ...
        aria-label="Resumen de aprobación curricular"
      >
        <div className="rounded-xl border border-ulima/25 bg-ulima/5 px-3.5 py-3">
          <p className="font-mono text-[9px] font-bold uppercase tracking-[0.1em] text-ulima">
            Sin decisión
          </p>
      ...
      {modoPaquetes ? (
        ...
        <>
          <div className="mt-5 rounded-xl border border-line bg-fondo p-3.5">
            <label
              className="block text-xs font-bold text-ink"
              htmlFor={`buscar-propuestas-${idEjecucion}`}
            >
            ...
            <div
              ...
              role="group"
              aria-label="Filtros de propuestas curriculares"
            >
              {FILTROS.map((opcion) => (
                <button
                  key={opcion.id}
            ...
            <p className="mt-2 text-xs leading-5 text-muted" aria-live="polite">
              Mostrando {filasVisibles.length} de {filas.length}{" "}
              {plural(
                filas.length,
                "propuesta pendiente",
                "propuestas pendientes",
              )}
              . Las deduplicadas automáticamente permanecen auditables y no
              entran en la cola de decisiones.
            </p>
          </div>
          ...
          {grupos.length ? (
            <div className="mt-5 space-y-4">
              {grupos.map((grupo) => (
                <section
                  key={grupo.tipo}
                  className="rounded-xl border border-line bg-fondo p-3.5"
                  aria-label={ETIQUETAS[grupo.tipo]}
                >
                      ...
                      <ProposalCard
                        ...
                        onImmediateAdd={
                          tipoDe(fila) === "competencia_tecnica"
                            ? (idPendiente) =>
                                aplicarDecisionTecnica(idPendiente, "ADD")
                            : undefined
                        }
                        onDiscard={
                          tipoDe(fila) === "competencia_tecnica"
            ...
            <div
              className="mt-4 rounded-xl border border-dashed border-line bg-fondo px-3.5 py-4 text-sm leading-6 text-muted"
              role="status"
            >
              No hay coincidencias para este filtro o búsqueda. Las propuestas
              sin decisión siguen pendientes y no se ocultaron de la cola.
            </div>
          )}
      ...
      {resultado ? (
        <div
          role="status"
          className="mt-5 flex items-start gap-3 rounded-xl border border-line bg-fondo px-3.5 py-3 text-ink"
        >
          <Check
            className="mt-0.5 shrink-0 text-ulima"
            size={18}
            aria-hidden="true"
          />
          <div>
            ...
            <p className="mt-1 text-sm leading-5">
              {resultado.accepted_in_request ?? resultado.accepted ?? 0}{" "}
              agregada
              {(resultado.accepted_in_request ?? resultado.accepted ?? 0) === 1
                ? ""
                : "s"}{" "}
              en esta acción y {resultado.remaining_pending ?? 0} propuesta
              {(resultado.remaining_pending ?? 0) === 1
                ? " permanece"
                : "s permanecen"}{" "}
              pendiente{(resultado.remaining_pending ?? 0) === 1 ? "" : "s"}.
            </p>
          </div>
        </div>
      ) : null}
      ...
      {resumen?.remaining_pending && !filas?.length ? (
        <p className="mt-4 text-xs leading-5 text-muted">
          Quedan {resumen.remaining_pending} propuestas mantenidas pendientes en
          los artefactos auditables de esta ejecución.
        </p>
      ) : null}
      ...
      {error ? (
        <div
          role="alert"
          className="mt-4 flex items-start gap-3 rounded-xl border border-red-200 bg-red-50 px-3.5 py-3 text-red-700"
        >
          <AlertTriangle
